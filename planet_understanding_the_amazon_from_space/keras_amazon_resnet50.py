...
def main_kfold():
    """Function to train, validate, predicting and submission with KFold."""
    random_state = 20
    train_dir = '../input/train-jpg'
    test_dir = '../input/test-jpg'
    ...
    preds_final_test = np.mean(preds, axis=0)
    preds_final_val = np.concatenate(pred_vals)
    pd.DataFrame(preds_final_test).to_csv('predictions_test_raw.csv',
                                          index=False)
    pd.DataFrame(preds_final_val).to_csv('predictions_val_raw.csv',
                                         index=False)
